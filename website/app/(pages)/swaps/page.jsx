...
const Employee = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch("http://localhost:9001/api/evUser/transactions");
  ...
  const fetchFilteredTransactions = async (phone) => {
    try {
      const response = await fetch(`http://localhost:9001/api/evUser/transactions/search/${phone}`);
      const data = await response.json();
